...
- **`OpenWhispererKit`** — pure, dependency-free, fast-to-test logic: `SentenceSplitter`, `NumberNormalizer`, `SubmitTrigger`, `VoiceSignal`, `VoiceMigration`, `PCMConversion`, `DisfluencyFilter`, `TTSVoiceRouter`.
...
- **Roster is a UI decision, not a bytes decision.** All 30 languages ride in the one model, so the picker's curated five (nl/de/pl/ru/uk × F1/M1) costs nothing to extend; the router accepts all 30 for `OW_TTS_VOICE`. F1–F5/M1–M5 are generic speaker *styles*, not accents — there is no Flemish-vs-Netherlands choice to expose, which is why Settings has **no** language/accent control (language rides inside the id, so a separate dropdown could only produce invalid pairs).
